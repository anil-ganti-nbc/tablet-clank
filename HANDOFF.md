...
The canonical database currently has integrity `ok`, 4 Apple/Samsung experimental sources, 169 products, 508 observations, 15 collector runs, 1420 rejected candidates, and 48 change events. All 48 are typed `identity_correction` (24 US, 24 IN), linked to observations, and reflect parser/identity repair after repeated carrier/unlocked URLs and a `wificell` normalization issue. `var/debug.db` is an ignored, non-canonical artifact from an earlier failed debugging run and must not be used as project state.
...
Fresh checkpoint command: `python -m pytest -q -rA`. Current result: 12 passed, 0 failed, 0 skipped, 0 xfailed.
...
Do not promote any source. Continue a small number of repeated experimental Apple Store monitoring cycles without promotion, then reassess long-term stability.
