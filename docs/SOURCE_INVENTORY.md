...
| Honor | `honor_uk_tablets` | UK | Honor UK tablet storefront catalogue (server-rendered HTML) | `HonorUKTabletsCollector` | **PRODUCTION-APPROVED (Promotion Wave 3, 2026-08-29)** — EXPERIMENTAL state retained per canonical identity pattern; isolated NAS campaign soak `honor-uk-iso-nas-001` COMPLETE 12/12 SUCCESS (Docker, Python 3.12.14/linux) | Canonical baseline established 2026-08-27 (Wave 2 validation) and re-verified silently at promotion; production approval granted after campaign evidence | **Production membership since 2026-08-29**, included exactly once in the production roster; campaign approval (`CAMPAIGN_APPROVED_SOURCE_IDS`) retired on promotion | NAS campaign 12/12: cycle 1 baseline 23 new, **0 events**; cycles 2–12 resight 23 / 0 new / **0 events**; 0 duplicates; integrity ok throughout; canonical DB byte-identical pre/post (evidence: NAS `state/logs/soak-honor-uk-iso-nas-001.jsonl` + snapshots). Promotion-day production cycle 2026-08-29: 23 accepted, all resights, **0 events**. Windows `honor-uk-iso-001` (2 healthy cycles) closed `OPERATOR_ABORTED_FOR_HOST_RELOCATION` — supporting evidence only, not counted | Slug identity (`honor.com` product slug); CN-lineage mapping unresolved and deliberately not fabricated |
