...
| Honor | `honor_uk_tablets` | UK | Honor UK tablet storefront catalogue (server-rendered HTML) | `HonorUKTabletsCollector` | EXPERIMENTAL; live-validated probe 2026-08-27 (HTTP 200, 0.5s, no JS); isolated NAS campaign soak `honor-uk-iso-nas-001` COMPLETE 12/12 SUCCESS 2026-08-29 (Docker, Python 3.12.14/linux) | Baseline+resight verified in isolated campaign DB; promotion to `PRODUCTION_ALLOWLIST` remains a separate explicit decision | Disabled; not allowlisted (`PRODUCTION_ALLOWLIST` unchanged) | NAS campaign 12/12: cycle 1 baseline 23 new, **0 events**; cycles 2–12 resight 23 / 0 new / **0 events**; 0 duplicates; integrity ok throughout; canonical DB byte-identical pre/post (evidence: NAS `state/logs/soak-honor-uk-iso-nas-001.jsonl` + snapshots). Windows `honor-uk-iso-001` (2 healthy cycles) closed `OPERATOR_ABORTED_FOR_HOST_RELOCATION` — supporting evidence only, not counted | Slug identity (`honor.com` product slug); CN-lineage mapping unresolved and deliberately not fabricated |
