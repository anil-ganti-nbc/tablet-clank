...
from dash_names import display_name, event_type_label
...
<td>{'<button class="btn collect-btn" data-source="' + esc(r['id']) + '">Collect Now</button>' if r['state'] == 'EXPERIMENTAL' else '<span class="muted">unavailable</span>'}</td>
    ...
    return f"""
<h1>Collect</h1>
<p class="pagesub">Live, single-source collection — mirrors the CLI's proven <span class="mono">collect &lt;source&gt; --live</span> path.</p>
{body}
<table><thead><tr><th>Source</th><th>Manufacturer</th><th>Region</th><th>Health</th><th>Last run</th><th>Action</th></tr></thead>
<tbody>{row_html}</tbody></table>
<div id="collect-result"></div>
{_collect_script()}
"""
