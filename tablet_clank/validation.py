...
def validate(candidate) -> tuple[bool, str | None]:
    ...
    path = urlsplit(candidate.url).path.lower().rstrip("/")
    if path.endswith("/all-tablets") or path.endswith("/tablets"): return False, "generic_category_page"
    if candidate.manufacturer.lower() == "apple" and not (candidate.source_identifier or candidate.raw_values.get("sku")):
        return False, "no_stable_product_identifier"
